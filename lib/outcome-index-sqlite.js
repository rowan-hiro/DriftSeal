...
class OutcomeIndex {
  ...
  laneCatalog() {
    const rows = this.db
      .prepare(
        `SELECT lanes.name, lanes.description, lanes.added_at, lanes.inferred,
                COUNT(outcomes.id) AS outcome_count,
                COALESCE(SUM(CASE WHEN outcomes.reclaimed = 0 THEN 1 ELSE 0 END), 0)
                  AS visible_count
         FROM lanes
         LEFT JOIN outcomes ON outcomes.lane = lanes.name
         GROUP BY lanes.name, lanes.description, lanes.added_at, lanes.inferred
         ORDER BY lanes.name`
      )
      .all();
  ...
  persistFoldState(state, event, startByte, endByte) {
    ...
    if (event.id && state.records.has(event.id)) {
      ...
      const existing = this.db
        .prepare('SELECT ordinal, first_byte FROM outcomes WHERE id = ?')
        .get(event.id);
    ...
    this.db
      .prepare(
        `INSERT INTO events(start_byte, end_byte, type, outcome_id, ts, event_json)
         VALUES (?, ?, ?, ?, ?, ?)`
      )
      .run(
        startByte,
  ...
  outcomeStartEvents() {
    return this.db
      ...
      .all()
      .map((row) => parseJson(row.event_json, 'outcome start event'));
  }
  ...
  containsEventSequence(events) {
    ...
    for (const candidate of candidates) {
      const stored = statement
        ...
        .map((row) => parseJson(row.event_json, 'committed event'));
      if (
        stored.length === events.length &&
        stored.every((event, index) => isDeepStrictEqual(event, events[index]))
      ) {
        return true;
      }
    }
